  ...
  if (success) {
    return (
      <Message
        success
        header="Success!"
        content="You order and payment have been accepted"
        icon="star outline"
      />
    )
  }
    ...
    return (
      <Segment secondary color="teal" inverted textAlign="center" placeholder>
        <Header icon>
          ...
          No produducts in your cart. Add some!
        </Header>
        <div>
          {user ? (
            <Button color="orange" onClick={() => router.push('/')}>View Products</Button>
          ) : (
            <Button color="blue" href="/login" onClick={() => router.push('/login')}>Login to add products</Button>
          )}
        </div>
      </Segment>
    )
